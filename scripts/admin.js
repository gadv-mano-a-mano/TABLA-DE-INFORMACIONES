...
import { onAuth, signInGoogle, signOutUser, handleAuthRedirect, auth, storage } from "./firebase.js";
...
const ADMIN_EMAILS = CONFIG.adminEmails.map(e => e.toLowerCase());
const MAX_MB = 50;
...
async function loadManifest(){
  ...
  if (!manifest) manifest = defaultManifest(CONFIG.boardDurationMs);
...
function syncUIFromSelected(){
  ...
  if (kind === "image"){
    ...
    fileHint.textContent = `Imágenes recomendadas: JPG (máx ${MAX_MB}MB).`;
  } else {
    fileInput.accept = "video/mp4,video/webm";
    fileHint.textContent = `Videos recomendados: MP4 H.264/AAC (máx ${MAX_MB}MB).`;
  }
...
async function createManifestInStorage(){
  ...
  await writeManifest(defaultManifest(CONFIG.boardDurationMs));
  await loadManifest();
...
async function uploadToSlot(){
  ...
  if (file.size > MAX_MB * 1024 * 1024) throw new Error(`Archivo demasiado grande (máx ${MAX_MB}MB).`);
...
btnLogout.addEventListener("click", async () => { await signOutUser(); });

// UI wiring
slotSelect.addEventListener("change", syncUIFromSelected);
btnUpload.addEventListener("click", async () => {
  try{ await uploadToSlot(); } catch(e){ setProgress(0, "Error: " + (e?.message || e)); }
});
btnToggle.addEventListener("click", async () => {
  try{ await toggleSlot(); } catch(e){ setProgress(0, "Error: " + (e?.message || e)); }
});
btnDelete.addEventListener("click", async () => {
  try{ await deleteSlotFile(); } catch(e){ setProgress(0, "Error: " + (e?.message || e)); }
});
btnCreateManifest.addEventListener("click", async () => {
  try{ await createManifestInStorage(); } catch(e){ setProgress(0, "Error: " + (e?.message || e)); }
});
